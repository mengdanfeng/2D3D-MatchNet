function Extract_SIFT_ISS_Area_MultiScale(track_list, track_point_idx, images, cam_id, ...
...
for i=1:size(valid_points, 1)
    ...
    volume_center_idx(i) = find(in_volume_index == valid_point_idx(i));
...
%     uv_copy = uv;
    ...
    top_left_x = round(sift_uv(1) - half_size);
    top_left_y = round(sift_uv(2) - half_size);
    bottom_right_x = round(sift_uv(1) + half_size);
    bottom_right_y = round(sift_uv(2) + half_size);
