...
function [reproj_point_img_idx_list, reproj_point_img_uv_list] = ...
    proj3DPoints2ImgWithBlockChecking(images, cam_idx, camera_laser_idx, cam_global_poses, ...
    laser_global_poses, G_camera_image, G_camera_ins, pointcloud,  camera_intrinsics,camID,...
    grid_resolution,submapID)
...
%     z_length = bound_z_max - bound_z_min;
    ...
    occupied_grid_x = floor((reproj_in_img_points_denoised(1,:)-bound_x_min)./grid_resolution_x) + 1;
    occupied_grid_y = floor((reproj_in_img_points_denoised(2,:)-bound_y_min)./grid_resolution_y) + 1;
    occupied_grid_z = floor((reproj_in_img_points_denoised(3,:)-bound_z_min)./grid_resolution_z) + 1;
    ...
    intersected_rows = ones(size(reproj_in_img_points_denoised,2),1);
%     keypts_in_img_idx = find(keypoints(1,:)>=bound_x_min &&　keypoints(1,:)<=bound_x_max ...
%         && keypoints(1,:)>=bound_x_min &&　keypoints(1,:)<=bound_x_max ...)
    for k=1:size(reproj_in_img_points_denoised,2)
        if(mod(k,100) ==0)
            fprintf('  progress: %d/%d\n',k,size(reproj_in_img_points_denoised,2));
        end
        lineCoord = [reproj_in_img_points_denoised(:,k); camera_center_position]';
...
%         pcshow(pointCloud(reproj_in_img_points'));
        ...
        %disp(size(indexes,1));
        [C, ~, ~] = intersect(occupied_grid, single(indexes(end,:)),'rows');
    ...
    reproj_point_img_idx = reproj_point_img_idx(valid_index);    
...
%    DISPLAY
    ...
    hold off
    path=sprintf('/media/mengdan/data2/robotcar/grasshopper/lms_front_proj/gps_60m/2014-06-26-09-53-12/cam%d_aft_%02d_%03d_1.png',camID,submapID, imgID);
    saveas(gcf, path);
    %pause
end