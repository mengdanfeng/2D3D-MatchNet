...
%     reprojPts = reproj_point_img_uv_list{imgID}(correspondences(:,2),:);
    file_path = sprintf('%s/cam%d_%03d.txt', submap_sift_iss_gt_corr_dir, cam_id, imgID);
    fid = fopen(file_path, 'w');
    fprintf(fid, '%d\n', length(dist_idx));
    for i = 1 : length(dist_idx)
        sift_id = dist_idx(i);
        sift_uv = image_features{imgID}(1:2, sift_id)';
        iss_rel_id = idx(dist_idx(i));
        iss_id = ib(iss_rel_id);
        iss_uv = reproj_point_img_uv_list{imgID}(iss_rel_id);
