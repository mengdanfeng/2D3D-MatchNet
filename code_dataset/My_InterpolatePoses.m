function [poses] = My_InterpolatePoses(ins_file, pose_timestamps)
...
%     poses{i} = poses_array(:,(i-1)*4+1:i*4);
%   end
  
  poses = [positions_interp; quaternions_interp];
  
  figure();
  plot(poses(1,:),poses(2,:));
  hold on
  plot(poses(1,1),poses(2,1),'r*')  
  plot(poses(1,end),poses(2,end),'bo')
  
  txt1 = 'start point';
  text(poses(1,1),poses(2,1),txt1);
  
  txt2 = 'ending';
  text(poses(1,end),poses(2,end),txt2);
