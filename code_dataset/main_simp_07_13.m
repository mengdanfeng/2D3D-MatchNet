...
mat_file_name = {};
for mat_id=17:27
    mat_file_name{end+1} = mat_files(mat_id).name;
end

for i=1:length(mat_file_name)
    GeneratePatch(mat_file_name{i});
end
% mat_file_name = mat_files(4).name;
% GeneratePatch(mat_file_name);

function GeneratePatch(mat_file_name)
...
for submapID=1:num_submap-1
    ...
    fprintf(fid, '    Compute sift features.\n');
    for i=1:2        
    ...
    for i=1:2
        ...
        [track_point_idx{i}, track_list{i}] = removeCloseISSPoints(track_list{i}, track_point_idx{i}, submap_keypoints{submapID});
