...
int *getSubscripts(int X, int Y, int Z);
...
int *intersectedVoxelDataX; //storage for the indexes of the intersected voxels - dynamically allocated
int *intersectedVoxelDataY;
int *intersectedVoxelDataZ;
int addedVoxelCount;
...
plhs[2] = mxCreateNumericMatrix(storageSize,1,mxINT32_CLASS,mxREAL); // initializes to zeros
//plhs[0] = mxCreateNumericMatrix(storageSize,3,mxINT32_CLASS,mxREAL); // initializes to zeros
intersectedVoxelDataX = (int*)mxGetData(plhs[0]);
intersectedVoxelDataY = (int*)mxGetData(plhs[1]);
intersectedVoxelDataZ = (int*)mxGetData(plhs[2]);
...
intersectedVoxelDataX[0] = X;
intersectedVoxelDataY[0] = Y;
intersectedVoxelDataZ[0] = Z;
addedVoxelCount = 1;
...
while ((X!=Xend)||(Y!=Yend)||(Z!=Zend))
{
    ...
    if (addedVoxelCount>storageSize)
    {
        ...
        intersectedVoxelDataX = (int*)mxRealloc(intersectedVoxelDataX, sizeof(int)*storageSize);
        intersectedVoxelDataY = (int*)mxRealloc(intersectedVoxelDataY, sizeof(int)*storageSize);
        intersectedVoxelDataZ = (int*)mxRealloc(intersectedVoxelDataZ, sizeof(int)*storageSize);
    }
    intersectedVoxelDataX[addedVoxelCount-1] = X;
    intersectedVoxelDataY[addedVoxelCount-1] = Y;
    intersectedVoxelDataZ[addedVoxelCount-1] = Z;

}
// Update the size of the output matrix
intersectedVoxelDataX = (int*)mxRealloc(intersectedVoxelDataX, sizeof(int)*addedVoxelCount);
intersectedVoxelDataY = (int*)mxRealloc(intersectedVoxelDataY, sizeof(int)*addedVoxelCount);
intersectedVoxelDataZ = (int*)mxRealloc(intersectedVoxelDataZ, sizeof(int)*addedVoxelCount);
mxSetM(plhs[0], addedVoxelCount); //number of rows
mxSetN(plhs[0], 1); //number of columns
mxSetData(plhs[0], intersectedVoxelDataX); // update pointer to the matrix data
mxSetM(plhs[1], addedVoxelCount); //number of rows
mxSetN(plhs[1], 1); //number of columns
mxSetData(plhs[1], intersectedVoxelDataY);
mxSetM(plhs[2], addedVoxelCount); //number of rows
mxSetN(plhs[2], 1); //number of columns
mxSetData(plhs[2], intersectedVoxelDataZ);
}
...
{
    int *subscripts;
    subscripts = (int *)malloc(sizeof(int)*3);
    subscripts[0] = X;
    subscripts[1] = Y;
    subscripts[2] = Z;
