function [T_matrices, pose_timestamps] = getGlobalPoses1(ins_file, pose_timestamps)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if(~issorted(ins_timestamps))
      fprintf('  Error: Ins_timestamps not in ascend order\n');
      [ins_timestamps, ins_idx] = sort(ins_timestamps, 'ascend');
      ins_data{6} = ins_data{6}(ins_idx);
      ins_data{7} = ins_data{7}(ins_idx);
      ins_data{8} = ins_data{8}(ins_idx);
      ins_data{13} = ins_data{13}(ins_idx);
      ins_data{14} = ins_data{14}(ins_idx);
      ins_data{15} = ins_data{15}(ins_idx);      
  end
  
  if(~issorted(pose_timestamps))
      fprintf('  Error: Pose_timestamps not in ascend order\n');
      [pose_timestamps, pose_idx] = sort(pose_timestamps, 'ascend');
  end      
...
%       'last')-1,1);
  if(min(ins_timestamps(:,1))<=min(pose_timestamps))
      %lower_index = find(ins_timestamps(:,1)<=min(pose_timestamps), 1, 'last')-1;
      lower_index = find(ins_timestamps(:,1)<=min(pose_timestamps), 1, 'last');
      camera_lower_idx = 1;
  else
      lower_index = 1;
      camera_lower_idx = find(pose_timestamps > ins_timestamps(lower_index, 1), 1, 'first');
  end
  
  if(max(ins_timestamps(:,1))>=max(pose_timestamps))
      %upper_index = find(ins_timestamps(:,1)>=max(pose_timestamps), 1, 'first')+1;
      upper_index = find(ins_timestamps(:,1)>=max(pose_timestamps), 1, 'first');
      camera_upper_idx = length(pose_timestamps);
  else
      upper_index = length(ins_timestamps(:,1));
      camera_upper_idx = find(pose_timestamps < ins_timestamps(upper_index, 1), 1, 'last');     
  end
...
%   upper_index = min(upper_index, numel(ins_timestamps));
  ...
  xy_position = zeros(numel(pose_timestamps),2);
  for i=1:numel(pose_timestamps)
    T_matrices{i} = poses_array(:,(i-1)*4+1:(i)*4);
    xy_position(i,:) = T_matrices{i}(1:2,4)';
  end

%   %% visualize path
%   figure(1);
%   plot(xy_position(:,1),xy_position(:,2),'b.');
%   hold on
%   plot(xy_position(1,1),xy_position(1,2),'r*')  
%   plot(xy_position(end,1),xy_position(end,2),'bo')
%   
%   txt1 = 'start point';
%   text(xy_position(1,1),xy_position(1,2),txt1);
%   
%   txt2 = 'ending';
%   text(xy_position(end,1),xy_position(end,2),txt2);
