...
pointcloud_downsampled= downsample_target_pc(out_of_plane, target_pc_size);
...
for i=1:2
    [reproj_point_img_idx_list{i}, reproj_point_img_uv_list{i}] = ...
        ...
        laser_global_poses, G_camera_image{i}, G_camera_ins{i}, pointcloud_downsampled, camera_intrinsics{i});
end
...
%%
%transform pointcloud to center at zero
pointcloud_downsampled= transform_pointcloud(pointcloud_downsampled);

%%
%output to file
keypoints= [track_point_idx{1};track_point_idx{2}];
output_to_file(pointcloud_downsampled, keypoints, submapID);
...
scatter3(-pointcloud_downsampled(2,1:end), ...
         ...
         -pointcloud_downsampled(3,1:end), 1, 'b.');
hold on
scatter3(-pointcloud_downsampled(2,track_point_idx{1}), ...
         -pointcloud_downsampled(1,track_point_idx{1}),...
         -pointcloud_downsampled(3,track_point_idx{1}), 15, 'r*');
     
scatter3(-pointcloud_downsampled(2,track_point_idx{2}), ...
         -pointcloud_downsampled(1,track_point_idx{2}),...
         -pointcloud_downsampled(3,track_point_idx{2}), 15, 'g*');
