...
cnt=1;
idx=zeros(size(camera_timestamps{1},1),1);
for i=2:size(camera_timestamps{1},1)
    ...
    if ~(isnan(cam_global_poses{1}{i}(1,4)) || isnan(cam_global_poses{1}{i}(2,4)) || isnan(cam_global_poses{1}{i}(3,4)) || ...
         isnan(cam_global_poses{2}{i}(1,4)) || isnan(cam_global_poses{2}{i}(2,4)) || isnan(cam_global_poses{2}{i}(3,4))) && valid
       idx(cnt) = i; 
       cnt = cnt + 1;
    end
end
