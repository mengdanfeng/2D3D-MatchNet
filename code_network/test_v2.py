...
def test(load_version, sift_test_list, iss_test_list, submap_id):
    ...
    with tf.Session(config=config) as sess:
        ...
        for k in range(len(sift_submap_test_list)):
            ...
            txt_file_path = "%s/%03d/%s.txt" % (sift_iss_correspond_dir, submap_id, cam_id)
            ...
            with open(txt_file_path, "w") as file:
                for i in range(dist_array.shape[0]):
                    ...
                    min_dist_thres_id = 0
                    for ii in range(top_k-1,-1,-1):
                        if dist_array[i,min_dist_id[ii]] < dist_threshold:
                            min_dist_thres_id = ii + 1
                            break
                    min_dist_id = min_dist_id[0 : min_dist_thres_id]
...
#                        img_pcl_correspondences.append([sift_test_list[i], iss_test_list[min_dist_id]])
                    ...
                    #file.write('%d %d %d %d %d %d\n' % ((i+1), min_dist_id[0]+1, min_dist_id[1]+1,
                    #           min_dist_id[2]+1, min_dist_id[3]+1,min_dist_id[4]+1))
